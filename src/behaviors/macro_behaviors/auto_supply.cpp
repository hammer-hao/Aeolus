...
namespace Aeolus
{
    ...
    int AutoSupply::_numSupplyRequired(AeolusBot& aeolusbot)
    {
        ...
        int completed_pylons = 0;
        int pending_pylons = mediator.GetNumberPending(
            aeolusbot,
        ...
        // emergency recovery: no pylons and none pending
        if (completed_pylons == 0 && pending_pylons == 0)
            return 1;
        ...
        {
            if (pending_pylons < std::ceil(static_cast<float>(
                num_production_structures) / 2))
            {
        ...
        else if (num_production_structures == 0
            && supply_left <= 2
            && pending_pylons == 0)
        {
            return 1;

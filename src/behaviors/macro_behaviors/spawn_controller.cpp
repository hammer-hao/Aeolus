...
namespace Aeolus
{
	bool SpawnController::execute(AeolusBot& aeolusbot)
	{
		...
		// calculate the deficit (target proportion - current proportion) for each unit type,
		// then sort by deficit descending so we prioritize the unit we are most lacking.
		...
		for (const auto& spawn_type : m_army_composition_map)
		{
			...
			for (const auto& unit : all_own)
				if (unit->unit_type == spawn_type.first)
					num_this_unit++;
			float current_proportion = num_this_unit / (num_total_units + 1e-7f);
			deficits.push_back({ spawn_type.first, spawn_type.second - current_proportion });
		}
		...
		// walk the unit types in priority order and issue a single training command for the
		// highest-priority type we can build this step, then return. we only ever queue one
		// unit per execute -- the queue naturally stays saturated because execute runs often.
		for (const auto& candidate : deficits)
		{
			...
			// structural reasons we can't build this type right now (tech not up, or all of its
			// production is busy) -- fall through and try the next priority type.
			...
			// couldn't issue the command (e.g. no valid warp-in spot) -- not an affordability
			// problem, so try the next priority type instead of waiting.
			m_production_to_unit_map.clear();
		}
